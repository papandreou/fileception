...
const mockfs = require('mock-fs-papandreou');
...
function mockOutFs(mocks) {
    if (mockStack.length > 0) {
        let mockStackCopy = [].concat(mockStack);
        unmock();
        mockStack = mockStackCopy;
    }
    if (mocks) {
        mockStack.push(mocks);
    }
    mocks = _.merge.apply(_, [{}].concat(mockStack));
...
module.exports = function fileception(mockDefinition, promiseFactory) {
    ...
    if (typeof promiseFactory === 'function') {
        ...
        unmockAfterCurrentTest = () => unmock(true);
    } else {
